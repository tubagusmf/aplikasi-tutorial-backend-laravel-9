...
class Post extends Controller
{
    ...
    public function tambah_proses(Request $request)
    {
        ...
        if (!empty($image) && !empty($files)) {
            ...
            $filenamewithextension2  = $request->file('file')->getClientOriginalName();
            $filename2               = pathinfo($filenamewithextension2, PATHINFO_FILENAME);
            $input['nama_file2']     = Str::slug($filename2, '-') . '-' . time() . '.' . $files->getClientOriginalExtension();
            $destinationPath2        = './assets/upload/file/';
            $files->move($destinationPath2, $input['nama_file2']);
            // END UPLOAD
